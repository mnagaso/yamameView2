...
using std::cout;
using std::endl;
#include "ExtractInnerSurface.h"


ExtractInnerSurface::ExtractInnerSurface( int timestart, int timebottom, double data[data_length], int mult_times, int mountainWidth, int mountPeakNum)
: sample_end( timebottom )
{
	resetData(data);

	sample_start = timestart;
	average = calcAverage();
	times = mult_times;
...
void ExtractInnerSurface::resetData(double data_in[data_length])
{
...
void ExtractInnerSurface::findMountain()
{
...
void ExtractInnerSurface::mountainNumbering()
{
...
void ExtractInnerSurface::mountainWidthThreshold()
{
...
void ExtractInnerSurface::extractPeak()
{
	int peak_count = 0;
	if( mountain_num != 0)
...
void ExtractInnerSurface::eraceNoisePeak()
{
	if( mountain_num != 0)
	{
		for( int i = 0; i < mountain_num; i++)
...
double ExtractInnerSurface::calcAverage()
{
	//double temp_sum = 0;
	int calc_width = 1000;
